...
public class Ball {
        ...
        while (distance > 0) {
            ...
            if (resolveTableCollisions()) {
                ...
                if (this.ballType == BallType.CUEBALL)
                    OneCushionScore.addCollision(OneCushionScore.borderHit);
                ...
                if (resolveBallsCollision(otherBall)) {
                    ...
                    if (this.ballType == BallType.CUEBALL)
                        if (otherBall.ballType == BallType.RED)
                            OneCushionScore.addCollision(OneCushionScore.redBallHit);
                        else
                            OneCushionScore.addCollision(OneCushionScore.yellowBallHit);
     ...
     * @return true, если столкновение произошло
     */
    private boolean resolveTableCollisions() {
